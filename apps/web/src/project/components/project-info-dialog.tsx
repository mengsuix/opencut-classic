		...
		<Dialog open={isOpen} onOpenChange={onOpenChange}>
			<DialogContent onOpenAutoFocus={(event) => event.preventDefault()}>
				...
				<DialogBody className="flex flex-col">
					<InfoRow label={t("shell.duration")} value={durationFormatted} />
					<InfoRow
						label={t("shell.created")}
						value={formatDate({ date: project.createdAt })}
					/>
					<InfoRow
						label={t("shell.modified")}
						value={formatDate({ date: project.updatedAt })}
					/>
					<InfoRow
						label={t("shell.projectId")}
						value={
				...
				<DialogFooter>
					...
					<Button onClick={() => onOpenChange(false)}>{t("shell.done")}</Button>
				</DialogFooter>
			</DialogContent>
		</Dialog>
	);
