...
7. 需要回滚（撤销之前的修改）时，必须先调 history.list 查看操作历史（每条含来源 user/agent、影响对象、时间），确认要回到的位置后用 history.jumpTo 跳到该位置；禁止不看历史直接连调 history.undo——栈顶可能是用户自己的操作，且回滚后再做任何新修改会永久丢失被撤销的内容（无法 redo 恢复），回滚范围不明确时先向用户确认
